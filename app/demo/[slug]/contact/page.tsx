...
export async function generateMetadata({ params }: DemoSubPageProps): Promise<Metadata> {
  ...
  return {
    ...
    title: config ? `Kontakt | ${config.restaurantName}` : "Kontakt"
  };
}

export default async function RestaurantDemoContactPage({ params }: DemoSubPageProps) {
  const { slug } = await params;
  const config = await getPublishedRestaurantDemo(slug);
  ...
  return <RestaurantDemoTemplate config={config} page="contact" slug={slug} />;
}
