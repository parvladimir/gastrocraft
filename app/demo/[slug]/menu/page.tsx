...
export async function generateMetadata({ params }: DemoSubPageProps): Promise<Metadata> {
  ...
  return {
    ...
    title: config ? `Speisekarte | ${config.restaurantName}` : "Speisekarte"
  };
}

export default async function RestaurantDemoMenuPage({ params }: DemoSubPageProps) {
  const { slug } = await params;
  const config = await getPublishedRestaurantDemo(slug);
  ...
  return <RestaurantDemoTemplate config={config} page="menu" slug={slug} />;
}
